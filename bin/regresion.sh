...
(cd $rutaap; RAILS_ENV=test bin/rails db:drop db:setup db:seed sip:indices)
if (test "$?" != "0") then {
...
(cd $rutaap;  ${RAILS} db:environment:set RAILS_ENV=test; RAILS_ENV=test ${RAILS} db:drop db:create db:setup db:seed sip:indices)
if (test "$?" != "0") then {
...
if (test "$CI" = "") then { # Por ahora no en gitlab-ci
  (cd $rutaap; CONFIG_HOSTS=127.0.0.1 ${RAILS} sip:stimulus_motores test:system)
  if (test "$?" != "0") then {
...
if (test -f $rutaap/bin/pruebasjs) then {
  (cd $rutaap; CONFIG_HOSTS=127.0.0.1 ${RAILS} sip:stimulus_motores; bin/pruebasjs)
  if (test "$?" != "0") then {
...
if (test "$rutaap" = "test/dummy/") then {
  ${RAILS} app:sip:reporteregresion
} else {
  ${RAILS} sip:reporteregresion
} fi;
r=$?
if (test "$r" != "0") then {
  exit $r;
