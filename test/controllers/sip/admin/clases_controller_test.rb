...
module Sip
  class ClasesControllerTest < ActionDispatch::IntegrationTest
    ...
    test "debe actualizar existente" do
      nuevaclase = Sip::Clase.create!(PRUEBA_CLASE)
      patch sip.admin_clase_path(nuevaclase.id),
        params: { 
          clase: { 
           id: nuevaclase.id,
            nombre: 'xyz'
      ...
      assert_redirected_to sip.admin_clase_path(assigns(:clase))
    end

    test "debe eliminar" do
      assert_difference('Clase.count', -1) do
        delete sip.admin_clase_path(Clase.find(1))
      end

      assert_redirected_to sip.admin_clases_path
    end
